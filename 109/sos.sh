...
shopt -u nullglob
printf "%s\n  ${Y}HTTP 502/503 BY DOMAIN (last %s)${X}\n" "$SEP" "$TW"
...
if [ -n "$DEV" ]; then
  ...
  printf "  ${Y}no block device found in /proc/diskstats${X}\n"
fi
printf "%s\n  ${Y}CROWDSEC METRICS${X}\n" "$SEP"
...
printf "%s\n  ${W}Rooted by VladiMIR | AI   v2026-04-13g${X}\n%s\n" "$SEP" "$SEP"
