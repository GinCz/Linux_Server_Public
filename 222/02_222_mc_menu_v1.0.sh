...
S       Cluster Status (st)
	clear
	/usr/local/bin/st 2>/dev/null || /usr/local/bin/stat_all 2>/dev/null || bash /root/Linux_Server_Public/monitoring/stat_all.sh 2>/dev/null || bash <(curl -fsSL https://raw.githubusercontent.com/GinCz/Linux_Server_Public/main/monitoring/stat_all.sh)
	read -p "Press Enter to exit..."
