#!/usr/bin/env bash
# =============================================================================
# block_bots.sh — Block IPs hammering wp-login/xmlrpc via iptables FIGHT_BOTS
# Version     : v2026-04-30
# Server      : 222-DE-NetCup (152.53.182.222)
# Usage       : fight  (alias) or bash /root/Linux_Server_Public/222/block_bots.sh
...
TRUSTED_IPS=("127.0.0.1" "8.8.8.8" "$REMOTE_IP" "152.53.182.222" "212.109.223.109")

touch "$BAN_DB" 2>/dev/null || { mkdir -p /root/scripts/state; touch "$BAN_DB"; }
...
main() {
  ...
  if [ "$n" -gt 0 ] && [ -n "${TG_TOKEN:-}" ]; then
    curl -s -X POST "https://api.telegram.org/bot${TG_TOKEN}/sendMessage" \
      ...
      -d "text=FIGHT (222): blocked ${n} IP(s):${list}" \
      -d "disable_notification=true" >/dev/null 2>&1 || true
  fi
  echo "=== FIGHT done. New bans: $n ==="
  [ "$n" -eq 0 ] && echo "No new IPs to block (threshold: $LIMIT requests)"
}
main
