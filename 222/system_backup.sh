...
clear
# =============================================================================
# system_backup.sh — Server 222 (EU Germany, NetCup)
# =============================================================================
# Version     : v2026-03-26
...
# -----------------------------------------------------------------------------
# Backup destinations:
#   PRIMARY   : local  /BACKUP/222/   (on this server)
#   SECONDARY : remote /BACKUP/222/   on 109 (xxx.xxx.xxx.109) via user vlad
# -----------------------------------------------------------------------------
# Archive includes:
#   /etc                        — all system configs
#   /root (selective)           — scripts, keys, configs (NO logs, NO git, NO docker data, NO backups)
#   /usr/local/fastpanel2       — FastPanel config
# Excludes: .git  session  cache  www  backups  crypto-docker/data  /tmp  logs
# =============================================================================
...
echo -e "$HR"
echo -e "${Y}   BACKUP — ${SERVER_NAME}  \u2192  local + 109${X}"
echo -e "$HR"
echo
...
if [ ${STATUS} -eq 0 ]; then
    MSG="\u2705 *BACKUP OK* | ${SERVER_NAME}%0A\ud83d\udce6 ${FILENAME}%0A\ud83d\udcca Size: ${SIZE}%0A\ud83d\udcbe local + 109"
else
    MSG="\u26a0\ufe0f *BACKUP PARTIAL* | ${SERVER_NAME}%0A\ud83d\udce6 ${FILENAME} — saved locally%0A\u274c Copy to 109 FAILED"
fi
