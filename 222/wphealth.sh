...
for SITE_DIR in /var/www/*/; do
    ...
    if [ -z "$WP" ]; then
        for TRY in "${SITE_DIR}wp-config.php" "${SITE_DIR}public_html/wp-config.php" "${SITE_DIR}www/wp-config.php"; do
            if [ -f "$TRY" ]; then WP="$TRY"; WP_ROOT=$(dirname "$TRY")/; break; fi
        done
    fi

    if [ -z "$WP" ]; then SKIP=$((SKIP+1)); continue; fi
