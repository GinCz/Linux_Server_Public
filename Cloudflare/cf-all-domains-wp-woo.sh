...
WHITELIST_EXPR='(ip.src eq 185.100.197.16) or (ip.src eq 185.14.233.235) or (ip.src eq 185.14.232.0) or (ip.src eq 90.181.133.10) or (ip.src eq xxx.xxx.xxx.222) or (ip.src eq xxx.xxx.xxx.109) or (ip.src eq 89.110.121.39) or (ip.src eq 144.124.228.237) or (ip.src eq 144.124.232.9) or (ip.src eq 144.124.228.227) or (ip.src eq 144.124.239.24) or (ip.src eq 195.63.138.33) or (ip.src eq 89.110.69.221) or (ip.src eq 144.124.233.38)'

# WP Extended + WooCommerce paths
WP_FIREWALL_EXPR='(http.request.uri.path eq "/wp-login.php") or (http.request.uri.path eq "//wp-login.php") or (http.request.uri.path eq "/xmlrpc.php") or (http.request.uri.path eq "//xmlrpc.php") or (http.request.uri.path eq "/wp-cron.php") or (http.request.uri.path eq "//wp-cron.php") or (http.request.uri.path eq "/wp-signup.php") or (http.request.uri.path eq "/wp-register.php") or (http.request.uri.path eq "/wp-trackback.php") or (http.request.uri.path eq "/wp-comments-post.php") or (http.request.uri.path contains "/wp-config") or (http.request.uri.path contains "/.env") or (http.request.uri.path contains "/.git") or (http.request.uri.path contains "/.htaccess") or (http.request.uri.path contains "/config.php") or (http.request.uri.path contains "/setup.php") or (http.request.uri.path contains "/install.php") or (http.request.uri.path contains "/upgrade.php") or (http.request.uri.path contains "/phpinfo") or (http.request.uri.path contains "/adminer") or (http.request.uri.path contains "/phpmyadmin") or (http.request.uri.path contains "/pma") or (http.request.uri.path contains "/mysql") or (http.request.uri.path contains "/wp-content/debug.log") or (http.request.uri.path contains "/wp-includes/ms-files.php") or ((starts_with(http.request.uri.path, "/wp-admin/") or starts_with(http.request.uri.path, "//wp-admin/")) and not (http.request.uri.path eq "/wp-admin/admin-ajax.php" or http.request.uri.path eq "//wp-admin/admin-ajax.php")) or (starts_with(http.request.uri.path, "/wp-json/") and (http.request.uri.path contains "/wp/v2/users" or http.request.uri.path contains "/wp/v2/settings" or http.request.uri.path contains "/wc/")) or (http.user_agent eq "") or (http.user_agent contains "sqlmap") or (http.user_agent contains "nikto") or (http.user_agent contains "nmap") or (http.user_agent contains "masscan") or (http.user_agent contains "zgrab") or (http.user_agent contains "python-requests") or (http.user_agent contains "go-http-client") or (http.user_agent contains "curl/") or (http.user_agent contains "libwww-perl") or (http.user_agent contains "WPScan") or (http.user_agent contains "Acunetix") or (http.user_agent contains "dirbuster") or (http.user_agent contains "nuclei") or (starts_with(http.request.uri.path, "/wc-api/")) or (starts_with(http.request.uri.path, "/checkout/")) or (starts_with(http.request.uri.path, "/my-account/")) or (starts_with(http.request.uri.path, "/cart/")) or (http.request.uri.query contains "wc-ajax=")'
...
rules=[
  ...
  {'description':'37-WP-WooCommerce-Challenge','expression':wp,'action':'managed_challenge','enabled':True}
]
print(json.dumps({'name':'VladiMIR WP+WooCommerce Security','kind':'zone','phase':'http_request_firewall_custom','rules':rules}))
...
echo -e "${C_YELLOW}  API sleep: ${API_SLEEP}s  |  Variant: WP + WooCommerce${C_RESET}"
echo -e "$SEP"
echo -e "${C_WHITE}  Rule 27 -- Whitelist Skip      (14 trusted IPs -> bypass all CF rules)${C_RESET}"
echo -e "${C_WHITE}  Rule 37 -- WP+Woo Firewall     (WP + WooCommerce paths + bad UA)${C_RESET}"
...
for ZONE_LINE in "${ZONE_LINES[@]}"; do
  ...
  echo -ne "  ${C_WHITE}Rule 27 + 37 (Whitelist + WP+WooCommerce)   ${C_RESET}"
  RES=$(apply_firewall_rules)
  if check "$RES" 2>/dev/null; then
    echo -e "  ${C_GREEN}  -> Rule 27: Whitelist OK  |  Rule 37: WP+WooCommerce OK${C_RESET}"
  else
    ZONE_FAILED=1
  fi
...
echo -e "  ${C_WHITE}Rule 37: WP + WooCommerce paths + bad UA -> CAPTCHA${C_RESET}"
