...
#
# Description: 3-layer Cloudflare security for ALL zones in account (auto-fetch).
...
rules=[
  ...
  {'description':'37-WP-Firewall-Challenge','expression':wp,'action':'managed_challenge','enabled':True}
]
...
for ZONE_LINE in "${ZONE_LINES[@]}"; do
  ...
  echo -ne "  ${C_WHITE}Rule 27 + 37 (Whitelist + WP Challenge)  ${C_RESET}"
  RES=$(apply_firewall_rules)
  if check "$RES" 2>/dev/null; then
    echo -e "  ${C_GREEN}  -> Rule 27: Whitelist Skip OK  |  Rule 37: WP Managed Challenge OK${C_RESET}"
  else
    ZONE_FAILED=1
  fi
  ...
  if [[ $ZONE_FAILED -eq 0 ]]; then
    echo ""
    echo -e "  ${C_GREEN}[OK]  ${DOMAIN} -- ALL RULES APPLIED${C_RESET}"
  else
    FAIL=$((FAIL + 1))
    echo ""
    echo -e "  ${C_RED}[!!]  ${DOMAIN} -- SOME RULES FAILED${C_RESET}"
  fi
done
