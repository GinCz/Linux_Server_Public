...
IPGUARD_DIR="$REPO_DIR/IPGuard"
LEGACY_DIR="$REPO_DIR/blacklist"
BLACKLIST_TXT="$IPGUARD_DIR/blacklist.txt"
BLACKLIST_CSV="$IPGUARD_DIR/blacklist-full.csv"
LEGACY_TXT="$LEGACY_DIR/blacklist.txt"
LEGACY_CSV="$LEGACY_DIR/blacklist-full.csv"
DATE=$(date +%Y-%m-%d)
...
# ==========================================================
# VladiMIR IPGuard Blacklist — Real Attack IPs
...
mkdir -p "$LEGACY_DIR"
cp "$BLACKLIST_TXT" "$LEGACY_TXT"

# ── [6] Write blacklist-full.csv ─────────────────────────
{
cat << CSVHEADER
# ==========================================================
# VladiMIR IPGuard Blacklist — Full Database (CSV)
...
# Mirror CSV to legacy path
cp "$BLACKLIST_CSV" "$LEGACY_CSV"

rm -f "$TMP_ALL" "$TMP_CSV"

# ── [7] Git commit + push ─────────────────────────────────
cd "$REPO_DIR"
git add IPGuard/blacklist.txt IPGuard/blacklist-full.csv blacklist/blacklist.txt blacklist/blacklist-full.csv

if git diff --cached --quiet; then
  echo "No changes since last update — nothing to commit."
else
  git commit -m "IPGuard: all-nodes update $DATE — $TOTAL unique IPs"
  ...
  echo " Primary URL: https://raw.githubusercontent.com/GinCz/Linux_Server_Public/main/IPGuard/blacklist.txt"
  echo " Legacy URL : https://raw.githubusercontent.com/GinCz/Linux_Server_Public/main/blacklist/blacklist.txt"
  echo "================================================"
fi
