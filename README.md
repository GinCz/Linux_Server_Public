...
**Версия — только внутри файла, никогда в имени файла.**

✅ Правильно: `backup_clean.sh`
❌ Неправильно: `backup_clean_v2026-04-25.sh`

Версия прописывается внутри скрипта:
```bash
# Version: v2026-04-29
```

История всех версий хранится в Git. Восстановить любую предыдущую версию:
```bash
git log -- 222/backup_clean.sh          # история коммитов
git show <sha>:222/backup_clean.sh      # содержимое конкретной версии
```

### Структура папок
| Папка | Назначение |
|---|---|
| `222/` | Сервер NetCup DE (152.53.182.222) |
| `109/` | Сервер FastVDS RU (212.109.223.109) |
| `VPN/` | VPN сервера (X-ray / Amnezia) |
| `scripts/` | Универсальные скрипты для всех серверов |
