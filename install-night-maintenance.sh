...
# After reboot (@reboot) — audit → Telegram report
...
tg "🔄 <b>${S}</b>: update done (${PKG_COUNT} pkg). Rebooting..."
sleep 3
/sbin/reboot
EOF
...
# = Rooted by VladiMIR + AI | v.2026.05.26 | github.com/GinCz =
#
# night-audit — post-reboot health check + Telegram report
# Usage: called by cron @reboot (runs once after each boot)
...
# --- Collect system info ---
UPTIME=$(uptime -p)
RAM=$(free -m | awk '/^Mem:/{printf "%dMB/%dMB (%.0f%%)", $3,$2,($3/$2)*100}')
...
[ -z "$FAILED" ] && SVC_STATUS="✅ all OK" || SVC_STATUS="❌ FAILED: $FAILED"

# --- Build report ---
MSG="✅ <b>${S}</b> rebooted successfully
⏱ ${UPTIME}
🧠 RAM: ${RAM}
💾 Disk: ${DISK}
⚡ Load: ${LOAD}
...
[ -n "$AUDIT_RESULT" ] && MSG="${MSG}
📋 Audit: ${AUDIT_RESULT}"

tg "$MSG"
echo "$(date '+%Y-%m-%d %H:%M:%S') REPORT SENT" >> "$LOG"
EOF
...
# --- Update crontab ---
# Remove old entries, set new unified schedule
