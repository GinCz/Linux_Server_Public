...
#
# Aliases: sos / sos1 / sos3 / sos24 / sos120 / sos 2h
# -----------------------------------------------
...
draw_bar(){
  ...
  local col
  [ "$pct" -ge 90 ] && col="$R" || { [ "$pct" -ge 60 ] && col="$Y" || col="$G"; }
  local bar="${col}[" i
...
# FastPanel2 detection (Ubuntu 24 path)
FP_DETECTED=0
for FP_CHECK in /usr/local/fastpanel2 /etc/fastpanel2 /opt/fastpanel2 \
                /var/lib/fastpanel2 /usr/share/fastpanel2; do
  [ -e "$FP_CHECK" ] && { FP_DETECTED=1; break; }
done
[ "$FP_DETECTED" -eq 0 ] && \
  systemctl list-units --type=service --all 2>/dev/null \
  | grep -qE 'fastpanel|fpanel' && FP_DETECTED=1
...
RAM_TOTAL=$(echo "$RAM_INFO" | awk '{print $1}'); RAM_TOTAL="$(safe_int "$RAM_TOTAL")"
RAM_USED=$(echo  "$RAM_INFO" | awk '{print $2}'); RAM_USED="$(safe_int "$RAM_USED")"
...
printf "  ${C}RAM:${X}  %s  ${W}%s used / %s total (free %s)${X}\n" \
  "$(draw_bar "$RAM_USED" "$RAM_TOTAL")" "$RAM_USED_H" "$RAM_TOTAL_H" "$RAM_FREE_H"
...
if [ "$ROLE" = "WEB" ]; then
  ...
  if [ -n "$FONT_HITS" ]; then
    ...
    echo "$FONT_HITS" | while read -r LOGFILE; do
      ...
      CNT=$(grep -ic 'File name too long' "$LOGFILE" 2>/dev/null | grep -c fonts 2>/dev/null || \
            grep 'File name too long' "$LOGFILE" 2>/dev/null | grep -ic fonts)
      CNT="$(safe_int "$CNT")"
...
if [ "$FP_FOUND" -eq 0 ]; then
  ...
  for D in /usr/local/fastpanel2 /opt/fastpanel2 /var/lib/fastpanel2 \
            /usr/share/fastpanel2 /etc/fastpanel2 /home/fastpanel2; do
    [ -e "$D" ] && { FP_PATH="$D"; break; }
  done
  if [ -n "$FP_PATH" ]; then
    ...
    if [ -n "$FOUND_UNIT" ]; then
      printf "  ${C}Related units:${X}\n"
      echo "$FOUND_UNIT" | while read -r U; do
        U_NAME=$(echo "$U" | sed 's/\.service//')
        STATE=$(systemctl is-active "$U_NAME" 2>/dev/null)
        [ "$STATE" = "active" ] && SC="$G" || SC="$R"
