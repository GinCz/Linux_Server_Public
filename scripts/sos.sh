...
HTTP_301=0
HTTP_502=0
for DIR in /var/log/nginx /var/log/apache2; do
    if [ -d "$DIR" ]; then
        C301=$(cat $DIR/*access.log 2>/dev/null | awk '$9 == 301 || $9 == 302' | wc -l)
        C502=$(cat $DIR/*access.log 2>/dev/null | awk '$9 == 502 || $9 == 503' | wc -l)
        HTTP_301=$((HTTP_301 + C301))
        HTTP_502=$((HTTP_502 + C502))
    fi
done
...
if [ -f "$MAIN_SCRIPT" ]; then
    ...
    grep -q "core update" "$MAIN_SCRIPT" && echo " - Core updater: ACTIVE" || echo " - Core updater: INACTIVE"
    grep -q "plugin update" "$MAIN_SCRIPT" && echo " - Plugin updater: ACTIVE" || echo " - Plugin updater: INACTIVE"
    grep -q "theme update" "$MAIN_SCRIPT" && echo " - Theme updater: ACTIVE" || echo " - Theme updater: INACTIVE"
else
