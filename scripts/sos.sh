...
# =============================================================
# Script: sos.sh
# Version: v2026.08.08a
#
# Changes v2026.08.08a:
#   - feat: section 06 now shows explicit Protection Provider Summary
#     (Cloudflare Web-only detection, CrowdSec Engine/Bouncer, Fail2ban,
#     Local Firewall) instead of a single ambiguous "Firewall: inactive"
#   - feat: section 12b Site Status Review — explains that 301/302 is
#     not automatically a problem, while 502/503 requires investigation
#   - feat: section 21 CrowdSec now shows explicit Engine/Bouncer status
#     and a clear warning when the Engine itself is inactive
#   - fix: section 26 renamed to "SWAP DEVICES" + new "26b. SWAP USAGE
#     BY PROCESSES" — empty process list no longer implies no swap exists
#
...
if [ "$IS_INSTALLED" -eq 0 ]; then
  clear
  printf "%s\n" "$SEP"
  printf " ${W}SOS${X} ${Y}v.2026.08.08a${X} | ${C}%s${X} | ${G}%s${X}\n" \
...
printf "%s\n" "$SEP"
printf " ${W}SOS ${Y}%s${X} | ${G}%s${X} | ${Y}v.2026.08.08a${X}\n" "$TW" "$NOW"
...
printf "  ${C}--- Protection Provider Summary ---${X}\n"
CF_DOMS=$(find /var/www/*/data/logs/ -name "*-frontend.access.log" 2>/dev/null | xargs -n1 basename 2>/dev/null | sed "s/-frontend.access.log//" | sort -u | head -5)
CF_HIT=0; CF_TOT=0
for D in $CF_DOMS; do [ -z "$D" ] && continue; CF_TOT=$((CF_TOT+1)); curl -skI --max-time 2 "https://$D" 2>/dev/null | grep -qi "cf-ray\|server: cloudflare" && CF_HIT=$((CF_HIT+1)); done
if [ "$CF_TOT" -eq 0 ]; then printf "  ${C}Cloudflare HTTP/HTTPS:${X}      ${Y}unknown (no domains found)${X}\n"
elif [ "$CF_HIT" -gt 0 ]; then printf "  ${C}Cloudflare HTTP/HTTPS:${X}      ${G}DETECTED on %s/%s domains (Web only)${X}\n" "$CF_HIT" "$CF_TOT"
else printf "  ${C}Cloudflare HTTP/HTTPS:${X}      ${Y}NOT DETECTED${X}\n"; fi
printf "                                 ${Y}note: Cloudflare does NOT protect SSH/SMB/FTP/SMTP/IMAP/POP3${X}\n"
CS_ENG=$(systemctl is-active crowdsec 2>/dev/null); CS_ENG=${CS_ENG:-inactive}
CS_BNC=$(systemctl is-active crowdsec-firewall-bouncer 2>/dev/null); CS_BNC=${CS_BNC:-inactive}
F2B_ST=$(systemctl is-active fail2ban 2>/dev/null); F2B_ST=${F2B_ST:-inactive}
[ "$CS_ENG" = active ] && EC="$G" || EC="$R"
[ "$CS_BNC" = active ] && BC="$G" || BC="$R"
[ "$F2B_ST" = active ] && FC="$G" || FC="$R"
printf "  ${C}CrowdSec Engine:${X}            %s%s${X}\n" "$EC" "${CS_ENG^^}"
printf "  ${C}CrowdSec Firewall Bouncer:${X}  %s%s${X}\n" "$BC" "${CS_BNC^^}"
printf "  ${C}Fail2ban:${X}                   %s%s${X}\n" "$FC" "${F2B_ST^^}"
if have ufw; then printf "  ${C}Local Firewall (UFW):${X}       %s\n" "$(ufw status 2>/dev/null | head -1)"
else printf "  ${C}Local Firewall (UFW):${X}       ${Y}not installed${X}\n"; fi
...
H "12b. SITE STATUS REVIEW"
printf "  ${C}301/302:${X} likely HTTPS redirect, canonical URL, or domain migration -- not necessarily a problem\n"
printf "  ${C}502/503:${X} backend unavailable, PHP-FPM timeout, or service down -- REQUIRES investigation\n"
...
H "21. CROWDSEC"
printf "  ${C}CrowdSec Engine:${X}            ${G}%s${X}\n" "$(systemctl is-active crowdsec 2>/dev/null | tr a-z A-Z)"
printf "  ${C}CrowdSec Firewall Bouncer:${X}  ${G}%s${X}\n" "$(systemctl is-active crowdsec-firewall-bouncer 2>/dev/null | tr a-z A-Z)"
if [ "$(systemctl is-active crowdsec 2>/dev/null)" != "active" ]; then
  printf "  ${R}WARNING: CrowdSec Engine is not active -- no active protection, this is NOT all-clear${X}\n"
fi
if have cscli; then
  BANS=$(printf '%s\n' "$CS_DECISIONS_CACHE" | awk 'BEGIN{c=0}/^\|/{c++}END{print (c>0?c-1:0)}')
  BANS="$(safe_int "$BANS")"
...
H "26. SWAP DEVICES"
swapon --show 2>/dev/null | sed "s/^/  /" || printf "  ${Y}no swap devices${X}\n"
free -h | awk '/^Swap:/{print "  Swap total: "$2"  used: "$3"  free: "$4}'

H "26b. SWAP USAGE BY PROCESSES (top-5)"
