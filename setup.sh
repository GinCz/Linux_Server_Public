...
if [ -f /root/.server_env ]; then
    ...
    # Linux не любит нижнее подчеркивание в именах серверов
...
echo -e "${CYAN}>>> [2/7] Установка пакетов (Samba, ACL, Bench)...${NC}"
apt update && apt install -y ufw mc ncdu htop acl samba samba-common git curl sysbench > /dev/null
# Настройка Git, чтобы не спрашивал пароль
...
echo -e "${CYAN}>>> [5/7] ФИНАЛЬНЫЙ Конфиг Samba (после установки)...${NC}"
